...
def main():
    ...
    for i in args_list:
        ...
        for j in np.arange(nv):
            ...
            plot_pfl_cmp_turbmethods_2col(case, var, restime, c_max, c_min, d_max, depth)

def plot_pfl_cmp_turbmethods_2col(case, var, restime, c_max, c_min, d_max, depth):

    # input data directory
    dataroot = dir_in+'/'+case
    # paths of files
    paths = [dataroot+'/'+turbmethod_list[i]+'_dampV_'+restime+'/gotm_out.nc' for i in range(nm)]
    # initialize dataset
    ...
    figname = figdir+'/Pfl_cmp_turbmethods_'+var+'_'+restime+'.png'
